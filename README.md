...
Le projet s'appuie sur le **German Credit Data** (Statlog, UCI Machine Learning Repository), 1 000 dossiers de crédit avec une variable cible équilibrée de façon réaliste (700 bons payeurs, 300 en défaut, soit **30 % de défaut**, cohérent avec un portefeuille de crédit à la consommation risqué).

Ce choix répond à plusieurs contraintes du projet :

- **Un vrai jeu de données, pas une simulation.** Contrairement à une première version de ce projet construite sur des données entièrement synthétiques, le German   Credit Data permet de valider la méthodologie sur des relations réelles entre variables et défaut, avec leurs imperfections (bruit, variables incomplètes,  absence de revenu déclaré).
...
**2. Feature engineering orienté capacité de remboursement.** En l'absence de revenu brut dans le dataset, trois ratios sont reconstruits :
- **PTI** (mensualité / revenu) directement à partir du bracket "taux de mensualité" déjà présent dans les données ;
...
1. **Fiabiliser la collecte des variables comportementales**, en particulier le statut du compte
   courant et l'historique de crédit : leur pouvoir prédictif domine largement celui des
   caractéristiques du prêt demandé, ce qui doit orienter les priorités de vérification lors de
   l'instruction d'un dossier.
2. **Ne jamais déployer de PD sans étape de calibration explicite** lorsque le modèle est entraîné avec
...
3. **Conserver une zone de revue humaine** plutôt que d'automatiser 100 % des décisions : elle absorbe
...
4. **Remplacer les proxys LTI/DTI par des données réelles de revenu** avant tout usage opérationnel :
